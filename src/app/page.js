...
const SYLLABUS = [
  {
    ...
    description: "Everything that should feel normal before the car has even had a chance to embarrass you.",
    weight: 1.6,
    modules: [
      {
  ...
  },
  {
    ...
    description: "This is the bit that quietly decides whether the test feels controlled or like a series of avoidable disasters.",
    weight: 1.8,
    modules: [
      {
  ...
  },
  {
    ...
    description: "A very popular place for learners to discover they are either test ready or absolutely not.",
    weight: 2,
    modules: [
      {
  ...
  },
  {
    ...
    description: "The part that separates driving from simply pointing a car roughly in the correct direction.",
    weight: 1.95,
    modules: [
      {
  ...
  },
  {
    ...
    description: "Lovely little collection of things that ruin confidence when they are half-understood.",
    weight: 2.1,
    modules: [
      {
        title: "Roundabouts",
  ...
  },
  {
    ...
    description: "The slow stuff that somehow still manages to produce maximum stress.",
    weight: 1.75,
    modules: [
      {
  ...
  },
  {
    ...
    description: "The section where the car is moving quickly enough to make poor planning feel really expensive.",
    weight: 1.7,
    modules: [
      {
        title: "Hill starts",
  ...
  },
  {
    ...
    description: "This is the bit that actually matters when you’re sat next to the examiner trying not to overthink your own existence.",
    weight: 2.25,
    modules: [
      {
  ...
  },
  {
    ...
    description: "Where the polite little theory of your driving meets reality and starts arguing with it.",
    weight: 2.35,
    modules: [
      {
...
const STARTER_TICKETS = [
  {
    ...
    message: "I’m safe, but I’m probably a bit too cautious at mini roundabouts. Is hesitation alone enough to lose marks?",
    links: "",
...
export default function App() {
    ...
    const timeout = setTimeout(() => {
      try {
        localStorage.setItem(
          PROGRESS_KEY,
          JSON.stringify({ ratings, transmission: profile.transmission, updatedAt: new Date().toISOString() })
        );
  ...
  const scoring = useMemo(() => calculateScoring(ratings, profile.transmission || "manual"), [ratings, profile.transmission]);
      ...
      modules: section.modules
        .map((module) => ({
          ...
          skills: module.skills.filter((skill) => {
            ...
            const matchesRatingFilter = selectedRatings.length === 0 || selectedRatings.includes(ratingValue);
  ...
  function toggleRatingFilter(value) {
    setSelectedRatings((prev) => (prev.includes(value) ? prev.filter((item) => item !== value) : [...prev, value]));
  }
  ...
  function updateTransmission(value) {
    setProfile((prev) => ({ ...prev, transmission: value }));
  }
        ...
        {!profile.isSignedIn ? (
          ...
          <>
            <Header
              ...
              signOut={signOut}
            />
            {page === "dashboard" && <Dashboard setPage={setPage} scoring={scoring} profile={profile} />}
...
function LandingPage({ profile, setProfile, authMode, setAuthMode, onSubmit, onProviderClick }) {
  return (
    <div className="grid min-h-[88vh] items-start gap-5 lg:items-center lg:gap-8 lg:grid-cols-[1.05fr,0.95fr]">
      <section className="rounded-[28px] p-5 shadow-[0_20px_70px_rgba(71,119,143,0.08)] sm:rounded-[36px] sm:p-8" style={{ background: `linear-gradient(135deg, ${BRAND.white} 0%, ${BRAND.blueLight} 60%, ${BRAND.yellowLight} 100%)`, border: `1px solid ${BRAND.border}` }}>
      ...
      <section className="rounded-[28px] bg-white p-5 shadow-[0_20px_70px_rgba(71,119,143,0.08)] ring-1 sm:rounded-[36px] sm:p-8" style={{ borderColor: BRAND.border }}>
        <div className="mb-5 grid grid-cols-2 gap-2 sm:flex">
          <button onClick={() => setAuthMode("signin")} className="rounded-2xl px-3 py-2 text-sm font-bold sm:px-4" style={authMode === "signin" ? { backgroundColor: BRAND.navy, color: BRAND.white } : { backgroundColor: BRAND.blueLight, color: BRAND.navy }}>
            Sign in
          </button>
          <button onClick={() => setAuthMode("signup")} className="rounded-2xl px-3 py-2 text-sm font-bold sm:px-4" style={authMode === "signup" ? { backgroundColor: BRAND.navy, color: BRAND.white } : { backgroundColor: BRAND.blueLight, color: BRAND.navy }}>
            Create profile
          </button>
        </div>
        ...
        <p className="mt-2 text-sm leading-6" style={{ color: BRAND.slate }}>
          This is a scaffold for proper accounts later. Right now it saves locally, but the structure is ready for proper sign-in next.
        </p>

        <form onSubmit={onSubmit} className="mt-5 space-y-4 sm:mt-6">
          <div>
            <label className="mb-2 block text-sm font-bold" style={{ color: BRAND.navy }}>Name</label>
            <input value={profile.name} onChange={(e) => setProfile((prev) => ({ ...prev, name: e.target.value }))} placeholder="Francis" className="w-full rounded-2xl border bg-white px-4 py-3 text-sm outline-none" style={{ borderColor: BRAND.border }} />
          </div>
          <div>
            <label className="mb-2 block text-sm font-bold" style={{ color: BRAND.navy }}>Email</label>
            <input value={profile.email} onChange={(e) => setProfile((prev) => ({ ...prev, email: e.target.value }))} placeholder="you@example.com" className="w-full rounded-2xl border bg-white px-4 py-3 text-sm outline-none" style={{ borderColor: BRAND.border }} />
          </div>
          <div>
          ...
          <button type="submit" className="w-full rounded-2xl px-4 py-3 text-sm font-bold" style={{ backgroundColor: BRAND.navy, color: BRAND.white }}>
...
function InsightCard({ title, item, tone }) {
  ...
  return (
    <div className="rounded-3xl p-4 ring-1" style={{ backgroundColor: bg, borderColor: BRAND.border }}>
      <p className="text-xs font-black uppercase tracking-[0.2em]" style={{ color: accent }}>{title}</p>
      <h3 className="mt-2 font-black">{item.skill}</h3>
      <p className="mt-1 text-sm" style={{ color: BRAND.slate }}>{item.module}</p>
    </div>
  );
}
  ...
  return (
    <div className="space-y-6">
      <section className="grid gap-4 lg:gap-6 lg:grid-cols-[1.15fr,0.85fr]">
        <div className="overflow-hidden rounded-[24px] p-4 ring-1 shadow-[0_20px_60px_rgba(71,119,143,0.10)] sm:rounded-[32px] sm:p-8" style={{ background: `linear-gradient(135deg, ${BRAND.white} 0%, ${BRAND.blueLight} 58%, ${BRAND.yellowLight} 100%)`, borderColor: BRAND.border }}>
          <div className="flex items-start justify-between gap-3">
            <div>
          ...
          <div className="mt-4 flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
            ...
            <div className="rounded-[22px] p-4 shadow-sm backdrop-blur ring-1 sm:rounded-[28px] sm:p-5" style={{ backgroundColor: BRAND.white, borderColor: BRAND.border }}>
          ...
          <div className="mt-6">
            <div className="mb-2 flex items-center justify-between text-sm font-semibold" style={{ color: BRAND.slate }}><span>Pass likelihood</span><span>{scoring.score}%</span></div>
...
function ActionCard({ title, copy, button, onClick, tone }) {
  ...
  return (
    <div className="rounded-[24px] p-4 shadow-[0_20px_60px_rgba(71,119,143,0.06)] ring-1 sm:rounded-[32px] sm:p-5" style={{ ...styles[tone], borderColor: BRAND.border }}>
      <h3 className="text-xl font-black" style={{ color: BRAND.navy }}>{title}</h3>
      <p className="mt-2 text-sm leading-6" style={{ color: BRAND.slate }}>{copy}</p>
      <button onClick={onClick} className="w-full sm:w-auto mt-5 rounded-2xl px-4 py-3 text-sm font-bold" style={{ backgroundColor: BRAND.navy, color: BRAND.white }}>
        {button}
      </button>
    </div>
  );
    ...
    <div className="space-y-6">
      <section className="rounded-[24px] bg-white p-4 shadow-[0_20px_60px_rgba(71,119,143,0.08)] ring-1 sm:rounded-[32px] sm:p-6" style={{ borderColor: BRAND.border }}>
        ...
        <div className="mt-5 grid gap-3 lg:mt-6 lg:grid-cols-[1fr,auto]">
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search skills or sections" className="rounded-2xl border px-4 py-3 text-sm outline-none placeholder:text-slate-400" style={{ borderColor: BRAND.border, backgroundColor: BRAND.white }} />
          <div className="flex flex-wrap gap-2">
            {STANDARD_LEVELS.map((level) => (
              <FilterChip key={level.value} label={level.short} active={selectedRatings.includes(level.value)} onClick={() => toggleRatingFilter(level.value)} />
            ))}
            <button onClick={clearRatingFilters} className="rounded-2xl px-3 py-2 text-xs font-black ring-1" style={{ backgroundColor: BRAND.white, color: BRAND.navy, borderColor: BRAND.border }}>
              Clear
            </button>
          </div>
        </div>
                  ...
                  <div className="space-y-4">
                    {section.modules.map((module) => (
                      <div key={module.title} className="rounded-[22px] p-4 ring-1 sm:rounded-[28px] sm:p-5" style={{ backgroundColor: BRAND.blueLight, borderColor: BRAND.border }}>
                        <h4 className="text-lg font-black" style={{ color: BRAND.navy }}>{module.title}</h4>
                                ...
                                <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                                  ...
                                  <div className="grid gap-2 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
                                    {levels.map((level) => {
                                      ...
                                      return (
                                        <button key={level.value} onClick={() => updateRating(skill.name, level.value)} className="rounded-2xl px-3 py-3 text-left text-xs font-bold transition" style={isSelected ? { backgroundColor: BRAND.navy, color: BRAND.white } : { backgroundColor: BRAND.white, color: BRAND.navy, border: `1px solid ${BRAND.border}` }}>
          ...
          <div className="rounded-[24px] bg-white p-4 shadow-[0_20px_60px_rgba(71,119,143,0.08)] ring-1 sm:rounded-[32px] sm:p-6" style={{ borderColor: BRAND.border }}>
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm" style={{ color: BRAND.slate }}>Questions appear here underneath the box, like support tickets.</p>
              </div>
            </div>
                ...
                <div key={ticket.id} className="rounded-[22px] p-4 ring-1 sm:rounded-[28px] sm:p-5" style={{ backgroundColor: BRAND.white, borderColor: BRAND.border }}>
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="text-xs font-semibold uppercase tracking-[0.18em]" style={{ color: BRAND.slate }}>{ticket.createdAt}</p>
                    </div>
                    <span className="rounded-full px-3 py-1 text-xs font-black" style={{ backgroundColor: ticket.replies.length ? BRAND.greenLight : BRAND.yellowLight, color: ticket.replies.length ? BRAND.green : BRAND.navy, border: `1px solid ${BRAND.border}` }}>
                  ...
                  <div className="mt-4 space-y-3">
                    {ticket.replies.length === 0 ? (
                      <div className="rounded-2xl p-3 ring-1" style={{ backgroundColor: BRAND.blueLight, borderColor: BRAND.border }}>
                        <p className="text-sm" style={{ color: BRAND.slate }}>No reply yet. Once answered, the response will appear directly underneath this question.</p>
                      </div>
                    ) : (
                      ticket.replies.map((reply) => (
                        <div key={reply.id} className="rounded-2xl p-3 ring-1" style={{ backgroundColor: BRAND.greenLight, borderColor: BRAND.border }}>
                          <p className="text-xs font-black uppercase tracking-[0.18em]" style={{ color: BRAND.green }}>Francis replied</p>
...
function ResourcesPage({ tipVideoIndices, learnVideoIndices, rerollTips, rerollLearn }) {
  return (
    <div className="space-y-6">
      <section className="rounded-[24px] p-4 shadow-[0_20px_60px_rgba(71,119,143,0.08)] ring-1 sm:rounded-[32px] sm:p-8" style={{ background: `linear-gradient(135deg, ${BRAND.white} 0%, ${BRAND.blueLight} 55%, ${BRAND.yellowLight} 100%)`, borderColor: BRAND.border }}>
        <p className="text-sm font-black uppercase tracking-[0.25em]" style={{ color: BRAND.navy }}>Video tips</p>
        <h2 className="mt-2 text-3xl font-black tracking-tight sm:text-5xl" style={{ color: BRAND.navy }}>Loads of help, built right in</h2>
  ...
  return (
    <section className="rounded-[24px] bg-white p-4 shadow-[0_20px_60px_rgba(71,119,143,0.08)] ring-1 sm:rounded-[32px] sm:p-6" style={{ borderColor: BRAND.border }}>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-sm font-black uppercase tracking-[0.25em]" style={{ color: BRAND.navy }}>{title}</p>
          <p className="mt-2 max-w-3xl text-sm leading-6" style={{ color: BRAND.slate }}>{copy}</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <button onClick={onRefresh} className="w-full sm:w-auto rounded-2xl px-4 py-3 text-sm font-bold" style={{ backgroundColor: BRAND.navy, color: BRAND.white }}>
            {buttonText}
          </button>
          <a href={`https://www.youtube.com/playlist?list=${playlistId}`} target="_blank" rel="noreferrer" className="w-full sm:w-auto rounded-2xl px-4 py-3 text-sm font-bold text-center" style={{ backgroundColor: BRAND.blueLight, color: BRAND.navy, border: `1px solid ${BRAND.border}` }}>
            Open full playlist
          </a>
        </div>
      </div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-2">
        {indices.map((index) => (
          <div key={`${playlistId}-${index}`} className="overflow-hidden rounded-[24px] bg-white ring-1" style={{ borderColor: BRAND.border }}>
            ...
            <div className="p-4">
              <p className="text-xs font-black uppercase tracking-[0.2em]" style={{ color: BRAND.navy }}>Playlist pick</p>
              <p className="mt-2 text-sm leading-6" style={{ color: BRAND.slate }}>
                Different random pick each time. Hit the button above for a fresh batch.
              </p>
            </div>
          </div>
        ))}
      ...
      <section className="rounded-[24px] bg-white p-4 shadow-[0_20px_60px_rgba(71,119,143,0.08)] ring-1 sm:rounded-[32px] sm:p-6" style={{ borderColor: BRAND.border }}>
        ...
        <form onSubmit={submitPost} className="mt-5 rounded-[22px] p-4 ring-1 sm:mt-6 sm:rounded-[28px] sm:p-5" style={{ backgroundColor: BRAND.blueLight, borderColor: BRAND.border }}>
          ...
          <div className="mb-4">
            ...
            <input value={newPost.subject} onChange={(e) => setNewPost((prev) => ({ ...prev, subject: e.target.value }))} placeholder="Would this be a fault? / Passed today / Test next week etc" className="w-full rounded-2xl border bg-white px-4 py-3 text-sm outline-none" style={{ borderColor: BRAND.border }} />
          </div>
          <div className="mb-4">
            <label className="mb-2 block text-sm font-bold" style={{ color: BRAND.navy }}>Topic</label>
            <select value={newPost.tag} onChange={(e) => setNewPost((prev) => ({ ...prev, tag: e.target.value }))} className="w-full rounded-2xl border bg-white px-4 py-3 text-sm outline-none" style={{ borderColor: BRAND.border }}>
